...
export const ViewCocktail = ({
  ...
  useEffect(() => {
    ...
    fetch(
      `http://localhost:8088/cocktails?id=${cocktailId}&_expand=user&_expand=method`
    )
      .then((res) => res.json())
      .then((res) => setCocktail(res[0]));

    fetch(
      `http://localhost:8088/cocktailIngredients?cocktailId=${cocktailId}&_expand=ingredient`
    )
      .then((res) => res.json())
      .then(setThisCocktailsIngredients);

    fetch(
      `http://localhost:8088/cocktailTypes?cocktailId=${cocktailId}&_expand=type`
    )
      .then((res) => res.json())
  ...
  const deleteCocktailIngredients = (obj) => {
    return fetch(`http://localhost:8088/cocktailIngredients/${obj.id}`, {
      method: "DELETE",
    });
  };
  const deleteCocktailTypes = (obj) => {
    return fetch(`http://localhost:8088/cocktailTypes/${obj.id}`, {
      method: "DELETE",
    });
  };
  ...
  return (
    <section
      className={`viewCocktail componentContainer ${theme ? "light" : "dark"}`}
      onClick={(e) => {
        setHamburger(true);
        setMyBarMenu(true);
      }}
        ...
        {thisCocktailsIngredients.map((cocktail, index) => (
          <li key={index} id={index}>
            ...
            <h3 style={{ display: "inline" }}>
              {` ${
                cocktail?.ingredient.ingredientTypeId === 3
                  ? "dashes"
                  : "ounces"
